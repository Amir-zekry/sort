...
async function BuyNow({ id }) {
    ...
    return (
        <section className="relative w-full bg-gradient-to-b from-black via-zinc-900 to-black py-16 px-6 text-center flex flex-col items-center justify-center space-y-6 overflow-hidden">
            {/* Background Glow */}
            ...
            <div className="relative z-10 space-y-3">
                <h2 className="text-2xl md:text-3xl font-bold text-white">
                    جاهز تشتري {item.name}؟
                </h2>
                <p className="text-gray-300 text-lg font-medium">
                    السعر: <span className="text-red-500 font-bold">{item.price} EGP</span>
                </p>
                ...
                <Button className="mt-4 text-lg bg-red-600 hover:bg-red-700 text-white font-semibold shadow-lg shadow-red-500/30 hover:scale-95 transition">
                    <Link href={`/checkout/${item.id}`} className="block px-6 py-2">
                        اشتري الآن 🚀
                    </Link>
                </Button>
            </div>
